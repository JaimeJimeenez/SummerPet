    ...
    hasAcceptedApplication(idOwner, idDogWatcher, callback) {
        this.pool.getConnection((err, connection) => {
            if (err) callback(new Error('Error de conexión a la base de datos: ' + err.message));
                ...
                const sql = 'Select count(*) from Application join UserApplication on IdOwner = ? and IdDogWatcher = ? and IdApplication = Id where Accepted = 1 and Active = 1 and curdate() < Application.FinalDate;';
